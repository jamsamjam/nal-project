...
def build_parser() -> argparse.ArgumentParser:
    ...
    default_output = script_dir / "selected_workload_cdfs.png"

    parser = argparse.ArgumentParser(
        description="Plot selected workload CDF files on a single graph."
    )
    parser.add_argument(
        "--input-dir",
        type=Path,
...
def main() -> None:
    ...
    workload_files = [input_dir / name for name in WORKLOAD_FILES]
    missing_files = [path for path in workload_files if not path.is_file()]
    if missing_files:
        missing = ", ".join(str(path) for path in missing_files)
        raise FileNotFoundError(f"Missing workload files: {missing}")
